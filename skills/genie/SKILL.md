...
- **Fast Refresh reset state / polluted a profile** → read `react_refresh_events` once and keep `latestSequence`; after the edit, call `react_refresh_events '{"afterSequence":N}'`. `preservedState` kept hook state, `remounted` lost it, and each row has the component id plus source. `filePaths` is best-effort and can be empty on Turbopack. Genie excludes `profileCommitsExcluded` from render aggregates; completion is one event that accounts for every unexpectedly reset component, or an empty `remounted` list that rules Refresh out.
...
- **Force a UI state without code edits** → `react_override_props`, `react_override_hook_state`, `react_override_context`. Hooks are labeled in `react_inspect_component` (`kind: state/reducer/memo/callback/ref/effect` + `stateIndex` on the stateful ones) — target by `stateIndex` (ordinal among stateful hooks, survives library-hook noise) or flat `hookIndex`; a wrong target errors with the full stateful-hook list to pick from.
- **Force a TanStack Query loading/error UI** → get an exact `queryHash` from `query_list`, then call `query_simulate_state '{"queryHash":"…","state":"pending"}'` or `query_simulate_state '{"queryHash":"…","state":"error","errorMessage":"Demo failure"}'`. Inspect/drive/screenshot the observer UI, then call `query_restore_state` with the same hash; completion is `query_get` returning the original status/data with no `simulatedState`. For Suspense or error-boundary presentation, use `react_toggle_suspense_fallback` / `react_force_error_boundary` instead. Use a controlled failing response plus `query_refetch` when testing retry, request, or `onError` behavior—the simulated state does not run a query function.
...
- **TanStack Query** → read with `query_list`/`query_get`/`query_get_data`; act with `query_invalidate`/`refetch`/`reset`/`set_data`/`simulate_state`/`restore_state`. Query tools appear when `<Genie />` can see a QueryClient — an explicit `<Genie queryClient={qc} />` prop, the router context, or any surrounding `QueryClientProvider`.
...
**CLI mechanics for agents**: `--fields id,name,renders` projects the result to JSONL (one object per row — top-level keys win when present, else the first array-of-records; zero matches = zero rows; unknown fields fail with the available names). `genie-react batch '[{"tool":…,"args":…},…]'` runs many calls on one connection (one JSON line each, continue-on-error, exit 1 if any failed). `--timeout <ms>` bounds one call (clamped 1s–120s). In `--json`, `--fields`, and batch modes, failures stay stdout-clean JSON with stable `status`, `reason`, `message`, `userActionRequired`, and safe `next` argv when applicable. `[busy]` means the app's main thread is blocked, not crashed: wait the returned `retryInMs` and re-call; `[timeout]`/`[not-connected]`/`[unknown-session]`/`[tool-error]` each name their own fix.
